...
public class RecipeInfoConstellation implements IRecipeInfoAltar {
	...
	public IRecipeType getRecipeType() {
		return RecipeTypeConstellation.INSTANCE;
	}
	...
	public void generateFromStacks(List<ItemStack> input, List<ItemStack> output, World world) {
		...
		patterns.clear();
		if(world != null) {
			TileAltar fakeAltar = new TileAltar(TileAltar.AltarLevel.CONSTELLATION_CRAFT);
			TileReceiverBaseInventory.ItemHandlerTile handler = fakeAltar.getInventoryHandler();
			int[] slotArray = RecipeTypeConstellation.SLOTS.toIntArray();
			for(int i = 0; i < 21; ++i) {
	...
	public boolean equals(Object obj) {
		if(obj instanceof RecipeInfoConstellation) {
			RecipeInfoConstellation other = (RecipeInfoConstellation)obj;
