...
public class TileAttunementCrafter extends TileBase implements ITickable, IPackageCraftingMachine, IStarlightReceiverLinkableTile, IGridHost, IActionHost {
	...
	public TileAttunementCrafter() {
		...
		if(Loader.isModLoaded("appliedenergistics2")) {
			hostHelper = new HostHelperTileAttunementCrafter(this);
		}
	}
	...
	protected String getLocalizedName() {
		return I18n.translateToLocal("tile.packagedastral.attunement_crafter.name");
	}
	...
	public boolean isBusy() {
		return isWorking || !inventory.stacks.subList(0, 13).stream().allMatch(ItemStack::isEmpty);
	}
	...
	protected void finishProcess() {
		...
		if(inventory.getStackInSlot(13).isEmpty()) {
			inventory.setInventorySlotContents(13, currentRecipe.getOutput());
		}
		else {
			inventory.getStackInSlot(13).grow(currentRecipe.getOutput().getCount());
		}
		for(int i = 0; i < 13; ++i) {
	...
	protected void chargeEnergy() {
		...
		ItemStack energyStack = inventory.getStackInSlot(14);
		if(energyStack.hasCapability(CapabilityEnergy.ENERGY, null)) {
			...
			if(energyStack.getCount() <= 0) {
				inventory.setInventorySlotContents(14, ItemStack.EMPTY);
			}
		}
	}
	...
	protected void matchStructure() {
		PatternBlockArray structure = StructureAttunementCrafter.INSTANCE;
	...
	public GuiContainer getClientGuiElement(EntityPlayer player, Object... args) {
		return new GuiAttunementCrafter(new ContainerAttunementCrafter(player.inventory, this));
	}

	@Override
	public Container getServerGuiElement(EntityPlayer player, Object... args) {
		return new ContainerAttunementCrafter(player.inventory, this);
	}
}
